  ...
  return res.status(500).json({code :500, message: "Campos incompletos"});
   ...
   pokemon.delete("/:id([0-9]{1,3})",async(req, res,next) => {
    ...
    if(rows.affectedRows ==1){
      return res.status(200).json({code :200, message: "Pokemon borrado completamente"});
    
    }
    return res.status(404).json({code :404, message: "Pokemom no encontrado"});
  ...
  pokemon.patch("/:id([0-9]{1,3})",async(req, res,next) => {
   ...
   pokemon.get('/:id([0-9]{1,3})',async (req,res,next) =>{
    ...
    return res.status(404).send({code: 404,message:"Pokemon no encontrados"});
  ...
  pokemon.get('/:name([A-Za-z]+)',async(req,res,next) =>{
   ...
   return res.status(404).send({code: 404, mesagge: "Pokemon no encontrado"});
   });
